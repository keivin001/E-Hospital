...
function DoctorDashboard({ user }) {
  ...
  return (
    <Layout title="Doctor Dashboard">
      {/* Welcome banner */}
      <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 100%)', borderRadius: 'var(--radius-xl)', padding: '24px', marginBottom: '24px', color: 'white', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '6px' }}>Good morning, {user?.name?.split(' ')[0]} 👨‍⚕️</h2>
              <p style={{ opacity: 0.85, fontSize: '0.85rem' }}>You have <strong>{todayApts.length} appointment{todayApts.length !== 1 ? 's' : ''}</strong> scheduled today.</p>
            </div>
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button className="btn" onClick={() => navigate('/chat')} style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', padding: '8px 16px', fontSize: '0.85rem' }}>
                <MessageSquare size={16} /> Messages
              </button>
              <button className="btn" onClick={() => navigate('/video')} style={{ background: 'white', color: '#047857', fontWeight: 700, padding: '8px 16px', fontSize: '0.85rem' }}>
